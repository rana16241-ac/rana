...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* Mega-menu (Services / Industries / Company) — same progressive-disclosure
     pattern on mobile: first tap opens the panel, second tap follows the link. */
  if (window.matchMedia('(max-width: 900px)').matches) {
    document.querySelectorAll('.mega-item').forEach(function (item) {
      var triggerLink = item.querySelector(':scope > a');
      if (triggerLink) {
        triggerLink.addEventListener('click', function (e) {
